  ...
  async updateCartItem(req, res) {
    ...
    try {
      ...
      const productsWithShipping = await Promise.all(cart.items.map(async (item) => {
        ...
        return { hasCustomShipping: prod?.hasCustomShipping, shippingPrice: prod?.shippingPrice || 0, quantity: item.quantity };
      }));

      const customShippingTotal = productsWithShipping.reduce((total, i) => {
        return i.hasCustomShipping ? total + (i.shippingPrice * i.quantity) : total;
      }, 0);

      if (customShippingTotal > 0) {
        deliveryCharge = customShippingTotal;
      } else if (cart.cartTotal < 2000) {
        deliveryCharge = 100;
      }
      ...
      res.status(200).json({
        ...
        finalTotal: Math.round(cart.cartTotal + deliveryCharge)
      });
